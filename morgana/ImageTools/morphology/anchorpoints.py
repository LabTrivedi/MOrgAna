...
def compute_anchor_points(ma,_slice,down_shape,smoothing=1000):
    ...
    midlinePoints = (midlinePoints/down_shape).astype(float)
    if midlinePoints.shape[0]==1:
        ...
        midlinePoints = np.transpose(midlinePoints)
    key_points = (np.array([k[0] for k in key_points])/down_shape).astype(float)
    ...
    anchors = np.array(points).astype(float)
    ...
    anch = np.concatenate((np.array([edge_point_L]),anchors[::s],np.array([edge_point_R])), axis=0).astype(float)
